...
- **`response.repairedRecommendations` and `response.truncated` now surface on every primary execution result** (`runAgent`, `runCommand`, `runWorkflow`, `runPipeline`, `run`), not only the low-level manual `client.submit()`. `UluOpsClient.trackIfEnabled` previously kept only `dashboardUrl` off the submission response and discarded both — the exact "computed and dropped at a boundary" defect this release's own `costUsd` fix names, reproduced one boundary further out in the fix that was supposed to close it (ship run #96, anxiety-reader F13).

- **The un-submittable `costUsd` warning is now PER-RUN, not per-client** (reverses ship #94's once-per-client suppression — Alex's call, ship run #95). A long-lived client's second and later runs each carry real, un-submitted cost; a once-ever warning went silent for every run after the first, so an operator reconciling a SPECIFIC run's spend against the tracker saw nothing on that run's own output.
...
### Fixed (score-aggregation-semantics spec v0.2.0 — the reach the 2026-08-24 fix left open)

- **A run that verified NOTHING no longer submits `averageScore: 0` to the tracker.** `aggregateScores` returns `0` for empty input on purpose (2026-08-24) so a GATED pipeline stage over an all-skipped workflow blocks instead of fail-opening on `null`; `PipelineExecutor.gateFailed` confirms "nothing ran" from execution counts before acting on it. But standalone — `runWorkflow()` with every phase skipped by `skip_if`/dependencies — or under a stage with no gate, no gate ever read that `0`, and `SubmissionClient`'s `result.score != null` guard forwarded it as a measured score: the tracker stored a scored `SHIP` run indistinguishable from a genuine failing one (tracker `748fcc02`, `1082d5fb`). `SubmissionClient.transformToOpsInput` now omits `averageScore` when the result's own metrics say `phasesExecuted === 0` / `stagesExecuted === 0` — the same positive-evidence check the gate uses, extracted to `src/utils/executionEvidence.ts` (`verifiedNothingExecuted`, internal, not exported). The run still submits: its agents, recommendations and skip structure are real data; only the fabricated number is withheld. A workflow that RAN a phase and scored `0` still submits `averageScore: 0` — pinned by a control test. **Semantics-without-signature:** `RunSubmission`'s wire shape is unchanged; the only observable difference is that this one class of run now arrives scoreless (`averageScore` absent → tracker stores `null`) where it used to arrive as `0`.
- **`PipelineExecutor.buildResult` no longer averages an ungated all-skipped stage in as a measured zero.** The submission check above reads the result `SubmissionClient` holds, and for a pipeline that is `stagesExecuted` — correctly `1` for a workflow-ref stage that ran and whose nested workflow skipped every phase — so the nested `0` was laundered into the pipeline's top-level score one level up: one such stage beside a genuine `85` rolled up to `43`. A stage whose result demonstrably executed nothing now enters the roll-up as scoreless (`null`); alone it yields a `null` pipeline score (omitted on submission), beside real stages it drops out of the average. A stage that ran and scored `0` still enters as `0`. Gates are untouched — `gateFailed` already handled this case. Found while building the spec's §7 L3-2 verification, which the spec's own §4 design could not satisfy; recorded here rather than folded silently.
- **`aggregateScores` doc-comment now states the rounding-boundary property** (tracker `e76c2577`, deferred 2026-08-21 into this spec; documentation only, no behaviour change): `average`/`weighted_average` round to an integer before the caller compares against an integer threshold, so at `DEFAULT_PASS_THRESHOLD` (75) a true mean of 74.5 passes and 74.4 fails. `min`/`max`/`sum` are exact.

### Fixed (ship run #96 — code-auditor's runtime-correctness audit and anxiety-reader's boundary-crossing read of ship #95's own fix batch, each reproduced against the pinned ops-sdk validator or the actual saved-run behavior before the fix)

- **`clampMaxScore`/`clampAverageScore` were NaN-transparent, re-opening the exact whole-run-abort ship #95 wrote them to close.** `NaN >= 0` is `false`, so a NaN score/maxScore fell into the clamp branch and `Math.max(0, Math.min(100, NaN))` is still `NaN` — the wire still rejects it. Both now reject non-finite values before the range check, matching `repairTokenCount`'s existing `!Number.isFinite` guard. Reachable via `UluOpsClient.submitRun`'s consumer-supplied `ExecutionResult` (a public-API input), not from core's own extraction, which is already NaN-free.
- **`keepSecondaryFailureCodes` threw a raw `TypeError`** on a non-array or non-string-containing `secondaryFailureCodes` — a new throw surface inside the one method documented as "a bad value can never abort the save." It fired on the offline (`trackingEnabled: false`) path too, since `createLocalResponse` routes through the same transform. Now array-guarded and filters non-string entries, repairing rather than throwing.
- **Truncation now leaves a trace on the wire.** A recommendation, agent, or analysis record dropped at the client-side ceiling (`MAX_RUN_AGENTS`/`MAX_RECOMMENDATIONS`/`MAX_ANALYSIS_RECORDS`) previously vanished with only a local log line — invisible to the tracker's own issue correlation, which reads a recommendation's absence as *resolved* and its reappearance next run as a *regression*. Every submission that drops anything now appends one stable, correlatable "Submission truncated at the client-side wire ceiling" recommendation (`PRA-FRA/C`) naming what was dropped, so a truncated run reads as incomplete instead of clean.
- **`safeParseYaml`'s scalar/sequence guard tests were shadowed by an unrelated downstream check.** Both ship-#95 tests used a `.agent.yaml` fixture, which routes through `renderLocally`'s independent `!agent` guard — removing `safeParseYaml`'s own `Array.isArray`/`typeof`-object checks still passed both tests. Isolated tests added on `.command.yaml` (whose `renderLocally` branch has no shape check of its own) that assert `safeParseYaml`'s own error text, so the guard can no longer be masked.

### Fixed (ship run #95 — code-auditor's egress census against the pinned ops-sdk validator, and a gate-polarity regression in ship #94's own fix, each verified against the pinned wire schema before the fix, each pinned by a test written first)

- **Five submission-boundary fields could abort an entire `runs.save` and lose the whole run record**, confirmed against the pinned ops-sdk client-side validator: an out-of-range `maxScore` (an agent's rubric exceeding 100), a negative or fractional `lineNumber`, a >20-entry or >20-char `secondaryFailureCodes`, an off-enum `classificationConfidence`/`classifiedBy`, and an unclamped `averageScore` (`aggregation.method: 'sum'` over several 90-scored agents legitimately exceeding 100). Each is now repaired or omitted at the submission boundary — never dropped silently, never forwarded raw — following the same `sanitizeRecommendation` policy as the fields ship #94 already covered (`SubmissionClient`).
- **`recommendations` had no client-side ceiling while its sibling arrays (`agents`, `analysisRecords`) did.** `PipelineExecutor` flattens recommendations across every stage with no per-stage limit; past the wire's `max(500)` the whole save aborted rather than truncating. Now capped with a warning, matching the sibling arrays.
- **Token counts were not integer-repaired before the wire**, which declares `.int()` on all nine `TokenUsageSchema` fields. A fractional or negative count (an averaged or apportioned metric) now rounds or is omitted rather than aborting the save.
- **`WorkflowExecutor.evaluateGate`'s ship #94 fail-closed fix overrode the author's declared `on_fail: warn`.** The unusable-threshold guard returned `'blocked'` BEFORE the `on_fail` check, so a phase whose author opted into a non-blocking gate was blocked anyway — and the fix's own comment claimed a polarity with the pipeline gate that the pipeline gate does not have (`PipelineExecutor.applyGate` always routes a failed gate through the author's declared posture). An unusable threshold is now a gate FAILURE, routed through `on_fail` like any other failure; `on_fail: warn` warns, everything else still fails closed. Shipped with no test in #94; both branches are pinned now.
- **`response.truncated`** reports, per submission, how many agents/recommendations/analysis records were dropped at the client-side wire ceilings — `{ agents: 0, recommendations: 0, analysisRecords: 0 }` when nothing was dropped. The truncation itself (`capWithWarning`) already existed and warned; there was previously no channel on the response for a caller to detect it programmatically.
- **`CRASH_PLACEHOLDER_VERSION` is now imported at all four sibling literal sites** (`PipelineExecutor.ts` x2, `WorkflowExecutor.ts`, `SubmissionClient.ts`'s `realVersion`) that ship #94 introduced the constant to unify but did not sweep — values agreed today, but a future change to the marker would have silently diverged at these sites.
- **Registry search widened from `limit: 10` to `limit: 100`** before the client-side exact-name filter in `RegistryClient.resolveRemote` — a real definition could rank past the old limit in a crowded namespace and never reach the filter, producing a false "not found" (unverified against the search API's own ranking guarantee; this widens the window rather than closing it).
- **Tests**: `max_parallel`/`gate.threshold` Infinity variants (the NaN guard's sibling boundary — `Number.isInteger(Infinity)` is already `false`, so this closes a coverage gap, not a defect), a bare-scalar and top-level-sequence YAML document (`safeParseYaml`'s other two guard arms), and both branches of the gate-polarity fix above.
