...
All 6 agent types (validator, executor, analyst, generator, explorer, forecaster) use a single `agentOutputSchema` and produce a unified `AgentResult`. There is no type-specific schema routing or result discrimination — categories and artifacts are both optional on every result. Decisions pass through as parsed, with one substitution: a missing decision becomes the literal `FAIL` (`AgentExecutor.ts:192`, `parsed.decision ?? 'FAIL'`). A low-confidence extraction is deliberately **not** substituted — the parsed decision is preserved and trust is carried separately on `extractionConfidence`/`extractionMethod`, with the gate call made downstream by `SubmissionClient.isPositiveDecision` rather than by erasing the decision here. `classifyDecision()` then normalizes to a `DecisionCategory` via vocabulary maps built from agent definitions.
...
All numeric defaults are in `constants.ts`: `DEFAULT_PASS_THRESHOLD` (75), `DEFAULT_WARN_THRESHOLD` (50), `DEFAULT_GATE_THRESHOLD` (70), `DEFAULT_MAX_STEPS` (50), `DEFAULT_MAX_TOKENS`, `DEFAULT_TEMPERATURE` (0), `DEFAULT_MAX_CONCURRENCY` (8), `DEFAULT_MODEL_ALIAS` ('sonnet').
...
### costUsd — computed at the AIProvider seam
...
| **Error propagation across layers** | Partially examined | The decision-propagation half is now traced and closed: every aggregation boundary (command wrap/aggregate, workflow aggregate, pipeline stage + `computeDecision`) carries and consumes `decisionCategory` via `resolveDecisionCategory()`, so custom-vocabulary and WDL-remapped negatives gate correctly end-to-end (tracker run #55, SEM-INC/H — the literal-`'FAIL'` inline-agent check was the concrete manifestation). Unstamped custom strings still resolve neutral: the vocabulary lives only with the producing executor, so category stamping at the producer is the invariant to preserve. The error/exception-propagation half (how thrown failures, not decisions, surface across layers) remains unexamined. |
...
| **costUsd unpriced vs. zero** | By design | costUsd is computed at the `AIProvider.buildGenerateResult` seam from registry pricing and rolled up via `sumCostUsd` at all four aggregation sites (shipped 2026-07-25). `undefined` means unpriced — unpriced registry row, unregistered model, offline fallback, or alias without a model — never a fabricated `$0`; `sumCostUsd` uses ANY-absent→undefined polarity so a partial sum is never presented as a total. Surviving tension: consumers must distinguish "unpriced" from "zero cost" and nothing in the type enforces that reading. |
| **`evidence_finding` provenance is partially, not fully, recoverable** | Accepted, partially mitigated | Records land on `evidence_finding` from four distinct paths — Tier 4 recommendation-derived, Tier 3 exploration-map-derived, a genuinely declared value, and the blank/missing-type fallback — with no single field naming which. Two of the four were already distinguishable before this was examined: Tier 4 carries `data.priority` + `failureMode` + `taxonomyVersion`; Tier 3 carries `data.sectionType` + `data.sectionLabel`. `sanitizeRecordType` now marks the remaining ambiguous pair — `data.recordTypeSource: 'fallback-blank'` distinguishes "agent declared nothing" from "agent declared `evidence_finding`" (2026-08-20). Pre-2026-08 rows predate the marker and carry none of this — they are permanently unattributable between the fallback and a genuine declaration. That is an accepted irreversible consequence of not having captured provenance from the start, not an open defect. |
